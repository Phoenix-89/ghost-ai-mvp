...
export default function Home() {
  useEffect(() => {
    gsap.from('.animate-fade-in', { opacity: 0, y: 50, duration: 1.5, ease: 'power2.out' });
    gsap.utils.toArray('.animate-quote-line').forEach((line, i) => {
      gsap.from(line, { opacity: 0, y: 20, duration: 1, delay: i * 0.3, scrollTrigger: { trigger: line, start: 'top 80%' } });
    });
    gsap.utils.toArray('.card').forEach((card, i) => {
      gsap.from(card, { opacity: 0, x: i % 2 === 0 ? -50 : 50, duration: 1, scrollTrigger: { trigger: card, start: 'top 80%' } });
    });
